...
export default function BuyoutInquiryForm({ selectedRange, onClearRange }: Props) {
    ...
    return (
      <div id="request" className="bg-cream/5 border border-cream/10 rounded-md p-8 text-center">
        ...
        <p className="text-cream/70 text-base leading-relaxed">
          We&apos;ve received your buyout request and will follow up within 48 hours.
        </p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      {selectedRange ? (
        ...
        <div className="bg-cream/5 border border-cream/20 border-dashed rounded-sm px-4 py-3">
          <p className="text-cream/50 text-sm">
            Select dates on the calendar above — 3-night minimum, 7-day lead time.
          </p>
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={state === 'loading' || !name || !email || !selectedRange}
