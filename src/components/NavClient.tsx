...
export default function NavClient({ linkStory, linkMission, linkTrips, ctaLabel, logoLightUrl, logoDarkUrl }: Props) {
  ...
  const logoSrc = isDetailPage ? logoDarkUrl : logoLightUrl

  // Nav background:
  // - Mobile menu open: always forest (so logo area isn't transparent/jarring)
  // - Detail pages scrolled: cream/95
  // - Homepage scrolled: forest/95
  // - Transparent: none
  const navBg = open
    ? 'bg-forest/97 backdrop-blur-sm'
    : scrolled
      ? isDetailPage
  ...
  const hamburgerClass = !open && isDetailPage && !scrolled ? 'text-forest' : 'text-cream'

  return (
    <div className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${navBg}`}>
      ...
      {/* Mobile menu — shares bg with nav wrapper above, no separate background needed */}
      {open && (
        <div className="px-6 pb-6 flex flex-col gap-5 md:hidden">
